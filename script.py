...
# Write to excel
pub_df.to_excel("pub_df.xlsx", index=False)
...
# Quick pub_dfs
pub_df["Page Number"].unique()
pub_df.columns.tolist()

pub_df.country.unique()
pub_df.category.unique()
pub_df.info()


# Assuming df is your DataFrame and "column_name" is the name of your column
unique_values = pub_df["category"].str.split(",").explode().unique()
unique_values

unique_values = pub_df["country"].str.split(",").explode().unique()
unique_values
...
# 4.0. Preparing Data to be used as JSON
# --------------------------------------

# Additional filter for JSON - we don't need author, origin_link, summary
evidence_df = df.filter(
    ["country", "title", "category", "date", "reliefweb_link", "thumb"]
)
